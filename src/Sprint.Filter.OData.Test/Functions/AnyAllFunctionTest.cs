...
namespace Sprint.Filter.OData.Test.Functions
{
    ...
    public class AnyAllFunctionTest
    {
        ...
        public void Any()
        {
            Assert.IsTrue(ExpressionEqualityComparer.Equals(Linq.Expr<Customer, bool>(t => t.Customers.Any(x => true)), Filter.Deserialize<Customer>("Customers/Any(x: true)")));

            Assert.IsTrue(ExpressionEqualityComparer.Equals(Linq.Expr<Customer, bool>(t => t.EnumerableCustomers.Any(x => true)), Filter.Deserialize<Customer>("EnumerableCustomers/Any(x: true)")));

            Assert.IsTrue(ExpressionEqualityComparer.Equals(Linq.Expr<Customer, bool>(t => t.ListCustomers.Any(x => true)), Filter.Deserialize<Customer>("ListCustomers/Any(x: true)")));
        }
