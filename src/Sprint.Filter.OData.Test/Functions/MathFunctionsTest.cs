...
namespace Sprint.Filter.OData.Test.Functions
{
    ...
    public class MathFunctionsTest
    {
        ...
        public void Round()
        {            
            Assert.IsTrue(ExpressionEqualityComparer.Equals(Linq.Expr<Customer, bool>(t => Math.Round(t.FreightDecimal) == 32), Filter.Deserialize<Customer>("round(FreightDecimal) eq 32")));

            // ReSharper disable once CompareOfFloatsByEqualityOperator
            Assert.IsTrue(ExpressionEqualityComparer.Equals(Linq.Expr<Customer, bool>(t => Math.Round(t.FreightDouble) == 32), Filter.Deserialize<Customer>("round(FreightDouble) eq 32")));
        }
