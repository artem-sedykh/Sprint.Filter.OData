...
// ReSharper disable once ConditionIsAlwaysTrueOrFalse
        ...
        {
            Assert.IsTrue(ExpressionEqualityComparer.Equals(Linq.Expr<Customer, bool>(t => !(t.FreightDoubleNullable > t.FreightDouble)), Filter.Deserialize<Customer>("not (FreightDoubleNullable gt FreightDouble)")));
            Assert.IsTrue(ExpressionEqualityComparer.Equals(Linq.Expr<Customer, bool>(t => !t.Name.EndsWith("milk")), Filter.Deserialize<Customer>("not endswith(Name,'milk')")));
        }
    }
}
